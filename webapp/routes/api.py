...
def process_metadata():
    try:
        ...
        return jsonify({"error": "upstream_unavailable", "message": "流程信息暂时不可用"}), 502
...
def start_sync():
    ...
    return jsonify({"job_id": job.job_id, "status": job.status, "message": job.message}), 202
...
def update_settings():
    ...
    try:
        ...
        updated = AppConfig(
            ...
            auto_sync=bool(data.get("auto_sync", current.auto_sync)),
